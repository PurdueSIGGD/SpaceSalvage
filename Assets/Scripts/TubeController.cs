	...
	void Update () {
		...
		if (!ejectkey) { //Stop everything if ejected
						...
						if (tubecount > 2) { //In order to pick up tubes
							if (timesincedrop > 3 && (Math.Abs (this.transform.position.x - ((Vector3)tubes [tubecount - 2]).x) < .8) && (Math.Abs (this.transform.position.y - ((Vector3)tubes [tubecount - 2]).y) < .8) && Vector3.Magnitude (transform.position - tubes[tubecount - 1]) < 2) {
											//print ("Picked up");
											tubecount--;
											dist = 0;
											tubesleft++;
						...
						if (dist > tubelength  && tubesleft > 0 && (  timesincelast > 4  || dist > tubelength * 1.3 )) { //can we drop it like it is hot?
